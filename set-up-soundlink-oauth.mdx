...
<Card title="soundlink-oauth-example" icon="github" href="https://github.com/Fan-ID/soundlink-oauth-example">
  Next.js app covering the PKCE connect flow, server-side code exchange, Client Credentials token generation, `userinfo`, and grant revocation — with tokens held server-side and every request triggered by an explicit action.
</Card>
...
| Client Credentials token \+ caching | `app/api/oauth/token/route.ts` |
