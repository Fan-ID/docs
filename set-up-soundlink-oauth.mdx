...
Every organization that will authorize your client must have the `THIRD_PARTY_INTEGRATIONS_ENABLED` feature flag switched on by Soundlink. It is **off by default**.

Until it is enabled, that organization will see:

| Where | Result |
| --- | --- |
| Consent approval | `403` — integrations disabled |
| Token request (`client_credentials`) | `400 invalid_grant` |
| Any request using a bearer token | `403 access_denied` |
...
| `403` | `access_denied` | Wrong grant type, or the organization's feature flag is off |
...
| `invalid_grant` | `400` | Code invalid, reused or expired; or no active grant; or feature flag off |
...
| `access_denied` | `403` (or redirect) | User declined; or wrong grant type; or feature flag off |
