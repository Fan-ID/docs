...
Client authentication or public metadata, not organization data.

| Endpoint | Notes |
| --- | --- |
| `GET /.well-known/oauth-authorization-server` | Public. No credentials. |
| `GET /api/v1/oauth/authorize` | Starts consent. `client_id` \+ PKCE, no token. |
...
<Note>
  This table covers the endpoints documented in this guide. `scopes_supported` in the discovery document is the authoritative list of scopes your client can request.
</Note>
