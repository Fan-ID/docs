...
A missing or unrecognized API key returns `401` with `invalid_api_key`. An invalid or expired OAuth token returns `401` with `invalid_token`. Do **not** send Firebase Bearer tokens or legacy product headers on this host.
...
<Note>
  `soundlinks:read` is minted on **new** keys only. Existing keys keep their original scopes — create a new key to get access. See [Syncing soundlinks](/soundlinks).
</Note>
