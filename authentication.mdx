...
Pass the full string in the **`Authorization`** header as a Bearer token:

```bash
-H 'Authorization: Bearer sk_abc123_...'
```

The same header also accepts an OAuth access token. See [OAuth Integration](/set-up-soundlink-oauth).

A missing or unrecognized API key returns `401` with `invalid_api_key`. An invalid or expired OAuth token returns `401` with `invalid_token`.
...
| `soundlinks:write` | Create and archive self-serve soundlinks | Yes |
...
Add `soundlinks:write` to [create](/creating-soundlinks) or [archive](/creating-soundlinks#archive-a-soundlink) soundlinks.
Add `campaigns:write` only when the integration should spend wallet credit or manage delivery.

<Note>
  Existing keys keep their original scopes — create a new key to get `soundlinks:read` or
  `soundlinks:write`. See [Creating soundlinks](/creating-soundlinks).
</Note>
...
| `POST /v1/soundlinks`                           | `soundlinks:write`           |
| `DELETE /v1/soundlinks/{soundlinkId}`           | `soundlinks:write`           |
