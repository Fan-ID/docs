...
Playlist engagement rows include `playlist_position` — the track's 1-based slot on the campaign playlist for that `report_date`. See [Playlist position](/playlist-position).
