...
// Country breakdown, engagement (warehouse grain), and JSONL exports:
// call the REST paths in [Soundlink metrics](/soundlink-metrics) and
// [JSONL exports](/jsonl-exports). Typed SDK helpers ship in a follow-up release.
```
...
| `soundlink.soundlinks.metricsTimeseries(id, params?)` | `GET .../metrics/timeseries` | Legacy daily aggregate — prefer REST `breakdown` |
| `soundlink.soundlinks.engagement(id, params?)` | `GET .../metrics/engagement` | Prefer REST warehouse-grain engagement until the next SDK release |
