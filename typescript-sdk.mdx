...
description: Install the soundlink SDK and call campaigns, soundlinks, write methods, and metrics from Node.js or Edge.
---
...
Requires `soundlinks:read`. Returns only soundlinks **not** linked to a campaign — campaign-backed links use the Campaigns API. Responses have no spend or fee fields (ads run in your Meta account).

```typescript
const { data: links, error: linksError } = await soundlink.soundlinks.list({
  page: 1,
  pageSize: 100,
  sortBy: "createdAt",
  sortOrder: "desc",
...
if (linksError) {
  console.error(linksError.code, linksError.message);
}

const { data: detail } = await soundlink.soundlinks.get("V1StGXR8_Z5jdHi6B-myT");
// detail?.autoFollow, detail?.metaPixelId, detail?.tiktokPixelId

const { data: overview } = await soundlink.soundlinks.metricsOverview(
  "V1StGXR8_Z5jdHi6B-myT",
  { startDate: "2026-08-01", endDate: "2026-08-31" },
);

const { data: breakdown } = await soundlink.soundlinks.breakdown.list(
  "V1StGXR8_Z5jdHi6B-myT",
  { startDate: "2026-08-01", endDate: "2026-08-31", pageSize: 50 },
);
```

Full REST guides: [Syncing soundlinks](/soundlinks) · [Soundlink metrics](/soundlink-metrics) · [Creating soundlinks](/creating-soundlinks). Scopes: [Authentication](/authentication).
...
See [Campaign metrics](/understanding-metrics) and [JSONL exports](/jsonl-exports).
...
Requires `soundlinks:read`. Self-serve soundlinks only (not campaign-linked). Breakdown and engagement helpers require **`soundlink` 2.2.0+**.

| Method | REST endpoint | Returns |
| ------ | ------------- | ------- |
| `soundlink.soundlinks.list(params?)` | `GET /v1/soundlinks` | `ApiResponse<SoundlinkListData>` |
| `soundlink.soundlinks.get(soundlinkId)` | `GET /v1/soundlinks/{id}` | `ApiResponse<SoundlinkDetail>` |
| `soundlink.soundlinks.metricsOverview(id, params?)` | `GET .../metrics/overview` | `ApiResponse<SoundlinkMetricsOverview>` |
| `soundlink.soundlinks.breakdown.list(id, params?)` | `GET .../metrics/breakdown` | `ApiResponse<SoundlinkBreakdownListData>` |
| `soundlink.soundlinks.breakdown.export(id, params?)` | `GET .../metrics/breakdown/export` | `ApiResponse<JsonlStream<SoundlinkBreakdownRow>>` |
| `soundlink.soundlinks.breakdown.export.collect(id, params?)` | Same export route | `ApiResponse<ExportCollection<SoundlinkBreakdownRow>>` |
| `soundlink.soundlinks.engagement.list(id, params?)` | `GET .../metrics/engagement` | `ApiResponse<SoundlinkEngagementListData>` |
| `soundlink.soundlinks.engagement.export(id, params?)` | `GET .../metrics/engagement/export` | `ApiResponse<JsonlStream<SoundlinkEngagementRow>>` |
| `soundlink.soundlinks.engagement.export.collect(id, params?)` | Same export route | `ApiResponse<ExportCollection<SoundlinkEngagementRow>>` |

`list` params: `page`, `pageSize` (max **100**), `sortBy` (`createdAt`), `sortOrder` (`asc` \| `desc`).
Breakdown and engagement list `pageSize` max is **500**. Export windows are capped at **90 days** per request.

Create and archive (`POST` / `DELETE /v1/soundlinks`) are not in the SDK. Use [Creating soundlinks](/creating-soundlinks).
...
Date params use inclusive `YYYY-MM-DD`. Export windows are capped at **90 days** per request. Breakdown list `pageSize` max is **500**. Paginated campaign engagement (`GET .../metrics/engagement`) is not in the SDK — use the HTTP path in [Campaign metrics](/understanding-metrics).
...
| **Soundlinks** | `SoundlinkSummary`, `SoundlinkDetail`, `SoundlinkListData`, `SoundlinkMetricsOverview`, `SoundlinkBreakdownRow`, `SoundlinkEngagementRow` |
| **Strategies** | `StrategiesCatalogData`, `StrategyCatalogItem`, `StrategyType`                                  |
| **Campaign metrics** | `MetricsOverview`, `BreakdownRow`, `BreakdownListData`, `EngagementRow`                          |
| **Params**     | `DateRangeParams`, `SoundlinkListParams`, `SoundlinkBreakdownListParams`, `SoundlinkEngagementListParams`, `BreakdownListParams`, `BreakdownExportParams`, `EngagementExportParams`, `IdempotencyOptions` |
| **Exports**    | `JsonlStream<T>`, `ExportCollection<T>`                                                          |
| **Client**     | `Soundlink`, `SoundlinkClientOptions`                                                            |

Row shapes match the warehouse schemas documented in [Campaign metrics](/understanding-metrics). For full field lists, use the [API Reference](/api-reference) or your IDE hover hints on the imported types.
...
[Syncing soundlinks](/soundlinks) → [Creating soundlinks](/creating-soundlinks) → [Creating campaigns](/creating-campaigns) → [Managing campaigns](/managing-campaigns) → [Campaign metrics](/understanding-metrics)
