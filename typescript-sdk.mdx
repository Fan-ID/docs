...
// For country breakdown, engagement, and JSONL exports, use the HTTP
// endpoints in [Soundlink metrics](/soundlink-metrics) and [JSONL exports](/jsonl-exports).
```
...
| `soundlink.soundlinks.engagement(id, params?)` | `GET .../metrics/engagement` | Use REST — see [Soundlink metrics](/soundlink-metrics) |

`list` params: `page`, `pageSize` (max **100**), `sortBy` (`createdAt`), `sortOrder` (`asc` \| `desc`).
Timeseries `pageSize` max is **500**. Soundlink breakdown and JSONL export methods are not in the SDK yet — use the HTTP paths in [Soundlink metrics](/soundlink-metrics) and [JSONL exports](/jsonl-exports).
