...
const { data: timeseries } = await soundlink.soundlinks.metricsTimeseries(
  "V1StGXR8_Z5jdHi6B-myT",
  { page: 1, pageSize: 50 },
);

// Engagement dates snap to Insights windows only (yesterday / last 7 days /
// all-time). Omitting startDate uses all-time — not createdAt.
const { data: engagement } = await soundlink.soundlinks.engagement(
  "V1StGXR8_Z5jdHi6B-myT",
  { sortBy: "totalStreams" },
);
```
...
See [Campaign metrics](/understanding-metrics) and [Campaign JSONL exports](/jsonl-exports).
...
| `soundlink.soundlinks.metricsTimeseries(id, params?)` | `GET .../metrics/timeseries` | `ApiResponse<SoundlinkTimeseriesData>` |
| `soundlink.soundlinks.engagement(id, params?)` | `GET .../metrics/engagement` | `ApiResponse<SoundlinkEngagementData>` |

`list` params: `page`, `pageSize` (max **100**), `sortBy` (`createdAt`), `sortOrder` (`asc` \| `desc`).
Timeseries `pageSize` max is **500**. Engagement `pageSize` max is **100**; date ranges snap to Insights windows (yesterday / last 7 days ending today / all-time).
...
| **Soundlinks** | `SoundlinkSummary`, `SoundlinkDetail`, `SoundlinkListData`, `SoundlinkMetricsOverview`, `SoundlinkDailyRow`, `SoundlinkEngagementItem` |
...
| **Params**     | `DateRangeParams`, `SoundlinkListParams`, `SoundlinkTimeseriesParams`, `SoundlinkEngagementParams`, `BreakdownListParams`, `BreakdownExportParams`, `EngagementExportParams`, `IdempotencyOptions` |
