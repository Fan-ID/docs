...
Both list and detail rows include `generation` — `3` only for wallet-funded campaigns, which are the ones you can [manage via the API](/managing-campaigns) (budget changes, tier updates, stop). Generations `1` and `2` are read-only via these write endpoints.
