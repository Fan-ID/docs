...
| -------------------- | -------------------------------- | ----------------------------------- |
| Rate limit           | **20 imports/hour** per API key  | `429` — honor `Retry-After`         |
...
<Note>
  The video import rate limit (20/hour) is stricter than the general Public API rate limit and is
...
| 429  | `rate_limit_exceeded`             | More than 20 imports in an hour (honor `Retry-After`)              |
