...
Reachability, content type, size, private/internal hosts, and download failures
typically surface as `status: failed` (with an `error` message) on
`GET /v1/videos/import/{sessionId}`, not as a synchronous `400` on `POST`.
