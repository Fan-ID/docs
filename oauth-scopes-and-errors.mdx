    ...
    "items": [
      {
        ...
        "playlistId": null,
        "trackName": "Yellow",
        "campaignUrl": "https://getsoundlink.com/orgs/org_xyz/insights/V1StGXR8_Z5jdHi6B-myT",
...
| `soundlinks:write` | Create and archive self-serve soundlinks |
...
<Note>
  `scopes_supported` in the [discovery document](/set-up-soundlink-oauth#discovery) is the authoritative list — it currently returns `openid`, `email`, `campaigns:read`, `campaigns:write`, `metrics:read`, `videos:write`, `soundlinks:read` and `soundlinks:write`.
</Note>
...
| `GET /v1/campaigns/{campaignId}/metrics/breakdown` | Per-day × country (`campaign_country_daily`). |
| `GET /v1/campaigns/{campaignId}/metrics/breakdown/export` | JSONL bulk for breakdown. |
| `GET /v1/campaigns/{campaignId}/metrics/engagement` | Per-day × track (`campaign_engagement_daily`). |
| `GET /v1/campaigns/{campaignId}/metrics/engagement/export` | JSONL bulk for engagement. |

### `soundlinks:read`

Self-serve soundlinks (no Soundlink-funded campaign). Same routes as the API key scope — see [Syncing soundlinks](/soundlinks) and [Soundlink metrics](/soundlink-metrics). `metrics:read` does not unlock these routes.
...
| `GET /v1/soundlinks/{soundlinkId}/metrics/breakdown` | Per-day × country (`soundlink_country_daily`). |
| `GET /v1/soundlinks/{soundlinkId}/metrics/breakdown/export` | JSONL bulk for breakdown. |
| `GET /v1/soundlinks/{soundlinkId}/metrics/engagement` | Per-day × track (`soundlink_engagement_daily`). |
| `GET /v1/soundlinks/{soundlinkId}/metrics/engagement/export` | JSONL bulk for engagement. |

### `soundlinks:write`

Same endpoints and rules as the API key scope of the same name — see [Creating soundlinks](/creating-soundlinks).

| Endpoint | Notes |
| --- | --- |
| `POST /v1/soundlinks` | Create a soundlink. Requires an `Idempotency-Key` header. Does not create a wallet campaign. |
| `DELETE /v1/soundlinks/{soundlinkId}` | Archive. No `Idempotency-Key`. |
...
| Endpoint | Notes |
| --- | --- |
| `POST /v1/videos/import` | Import a partner video. Separate rate limit (default 20 imports/hour per API key or OAuth grant). |
...
`400` (bad query), `401`, `403`, `404`, `429` and `500` are all possible. Campaign `404`s use `campaign_not_found` (missing or another organization's campaign). Soundlink `404`s use `not_found` (missing, another organization, or linked to a campaign).
