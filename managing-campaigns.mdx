...
Campaign list and detail responses include a `generation` field. Wallet-funded campaigns are **`generation: 3`** — the only ones these management endpoints accept. Calling a management endpoint on another generation returns `400 invalid_request` (for example, "Stopping is only supported for wallet campaigns.").
