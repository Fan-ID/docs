...
Node.js users can stream **campaign** exports with the [TypeScript SDK](/typescript-sdk) (`for await` over `metrics.breakdown.export`). Soundlink JSONL helpers land with the next SDK release — until then, call the HTTP paths above.
...
[Campaign metrics](/understanding-metrics) · [Soundlink metrics](/soundlink-metrics) · [Playlist position](/playlist-position) · [Errors](/errors)
