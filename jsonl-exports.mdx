---
title: Campaign JSONL exports
description: Bulk campaign metrics downloads for data pipelines without pagination.
---
...
Node.js users can stream the same exports with the [TypeScript SDK](/typescript-sdk) (`for await` over `metrics.breakdown.export`).

Treat each line as an upsert on the schema primary key (documented in [Campaign metrics](/understanding-metrics)). Re-running the same window after the 7-day mutability period is safe.
...
[Campaign metrics](/understanding-metrics) · [Playlist position](/playlist-position) · [Errors](/errors)
