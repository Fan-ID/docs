...
`POST /v1/soundlinks` creates a soundlink that is **not** tied to a Soundlink-funded campaign. Requires the `soundlinks:write` scope, and your organization must be enabled for soundlink create.
...
| 403 | `soundlinks_write_not_enabled` | Organization is not enabled to create soundlinks via the Public API |
