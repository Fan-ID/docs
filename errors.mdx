...
OAuth endpoints (`/api/v1/oauth/*`) use `{ "error": "<code>" }` with no `meta`. See [OAuth scopes and errors](/oauth-scopes-and-errors#errors-and-rate-limits).
...
| HTTP | Code                      | When                                        |
| ---- | ------------------------- | ------------------------------------------- |
| 401  | `invalid_api_key`         | Missing or malformed API key (`Authorization: Bearer`) |
...
| 429  | `rate_limit_exceeded`            | Stricter per-key limit: 20 imports/hour (honor `Retry-After`)      |

## Rate limits

Default Public API limits are **60 requests/minute** and **600 requests/hour**, keyed by API key or OAuth client. Video import is **20 imports/hour**, counted separately. Honor `Retry-After`.
